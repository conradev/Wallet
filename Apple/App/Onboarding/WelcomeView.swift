    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            Text(observable.viewModel.subtitle)
                ...
                .frame(maxWidth: 500)
            Spacer()
            ForEach(Array(observable.viewModel.options), id: \.self) { option in
...
extension WelcomeViewModel: KotlinViewModel {
    final class Observable: KotlinObservableObject<WelcomeViewModel> {
        @Published
...
extension WelcomeViewModel.Option: KotlinCaseIterable {
